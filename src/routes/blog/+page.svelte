<script lang="ts">
	...
	// results = derived value (no $effect mutation → no-unused-expressions goes away)
	const results = $derived.by<Item[]>(() =>
		query(allPosts, {
			...
			filter: {
				...
				keywordsAny: selectedKeywords.length ? selectedKeywords : undefined
			},
			sortBy: 'date',
			sortDir: 'desc'
		})
	...
	const toggleTech = (t: string) => (selectedTech = toggle(selectedTech, t));
	const toggleKeyword = (k: string) => (selectedKeywords = toggle(selectedKeywords, k));
